  ...
  return (
    <section id="education" className="py-20 px-4 md:px-6 max-w-4xl mx-auto border-b-1 border-b-gray-500/50">
      ...
      <div className="grid gap-6">
        {loading
          ? [1, 2].map((n) => <div key={n} className="h-32 bg-slate-800/50 animate-pulse rounded-xl border-l-4 border-slate-700" />)
          : sortedEducation.map((edu: Education, index: number) => (
              <ScrollRevealY key={edu.id} delay={index * 0.15}>
                <div className="group p-4 md:p-6 bg-gray-100/10 border-l-4 border-blue-400 rounded-r-xl hover:bg-blue-200/5 transition-all duration-300 flex flex-row gap-3 md:gap-6 items-center overflow-hidden">
