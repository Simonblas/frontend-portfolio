...
const SkillSection = () => {
  const { data: skills, loading, request: fetchSkills } = useApi(portfolioService.getSkills);
  ...
  return (
    <section id="skills" className="py-20 px-6 max-w-6xl mx-auto">
      <div className="text-center mb-16">
        <h2 className="text-3xl font-bold mb-2 text-center w-fit mx-auto italic">Skills</h2>
        <p className="text-slate-400 mt-0">My skills, organized by categories</p>
      </div>

      {loading ? (
        <div className="grid grid-cols-1 rounded-3xl md:grid-cols-2 gap-10 animate-pulse">
          {[1, 2, 3, 4].map((n) => (
            <div key={n} className="h-48 bg-slate-900/50 rounded-3xl border border-slate-800"></div>
          ))}
        </div>
      ) : (
                ...
                <div className="flex flex-wrap gap-2">
                  {categorySkills.map((skill) => (
                    ...
                    >
                      ...
                      {skill.iconoUrl ? (
                        <img src={skill.iconoUrl} alt={skill.nombre} className="w-6 h-6 object-contain" />
                      ) : (
                        <span className="text-sky-400">⚡</span>
                      )}

                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-200 group-hover:text-white">{skill.nombre}</span>
